...
class HFBacktest(object):
    def __init__(self, price, tradePoint='close'):
        self.factorDict = {}
        self.price = price
        self.tradePoint = tradePoint #成交时使用的价格
...
class HFSingleFactorBacktest(object):
    ...
    def analyze(self, layerNum=10, positionPct=0.1, turnoverLimit=0.5):
        ...
        self.groupBacktest(layerNum)  #分层组合回测
    
        self.ICBacktest()  #求IC值？？？？

        self.generateLongShortPosition(positionPct, turnoverLimit)  #生成多空仓仓位
            
        self.calcLongShortPerformance()  #计算多空仓表现
